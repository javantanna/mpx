...
# =============================================================================
# CORE ENCODER (MODIFIED for LSB Payload)
# =============================================================================
...
class MP5Encoder:
    ...
    def encode(
        ...
        # ---------------------------------------------------------------------
        # MODIFIED: Payload is now prepared for LSB, not Atom
        # ---------------------------------------------------------------------
        
        # 1. Prepare Atom Metadata (Public/Decoy Info)
        atom_metadata = {
            "mp5_version": self.config.version,
            "created": datetime.utcnow().isoformat() + "Z",
            "original_hash": original_hash,
            "video_info": video_info,
            "note": "Secure payload stored in LSB layer",
            "layers": {
                "atom": {
        ...
        # 2. Prepare LSB Metadata (The Actual AI Training Payload)
        lsb_metadata = {
            "mp5_version": self.config.version,
            "timestamp": atom_metadata["created"],
            "payload_type": "ai_training_data",
            "metadata": metadata  # <--- USER INPUT IS NOW HERE
        }
        ...
        if use_lsb:
            # Write LSB layer (THE PAYLOAD)
            logger.info("Writing LSB layer (Hidden Payload)...")
            temp_path = output_path + '.mp5_lsb_temp.mp4'
            self.lsb_layer.write(video_path, lsb_compressed, temp_path)
            
            # Write Atom layer (File Info)
            logger.info("Writing Atom layer (Headers)...")
            self.atom_layer.write(temp_path, atom_compressed, output_path)
            
            os.remove(temp_path)
        else:
            # Fallback if user disabled LSB (Not recommended for your use case)
            logger.warning("LSB disabled. Writing payload to Atom layer instead.")
            atom_metadata["metadata"] = metadata
            atom_compressed = self.compression.compress_json(atom_metadata)
        ...
        result = {
            ...
            "storage_method": "LSB Steganography" if use_lsb else "Atom"
        }
...
# =============================================================================
# CORE DECODER (MODIFIED to read LSB)
# =============================================================================
...
class MP5Decoder:
    ...
    def decode(
        ...
        # 1. Try LSB Layer (Primary Payload)
        logger.info("Scanning LSB layer for hidden payload...")
        try:
            lsb_compressed = self.lsb_layer.read(mp5_path)
            if lsb_compressed:
                lsb_data = self.compression.decompress_json(lsb_compressed)
                result["hidden_payload"] = lsb_data.get("metadata")
                result["lsb_info"] = lsb_data
                logger.info("✓ FOUND HIDDEN LSB PAYLOAD")
            else:
                logger.warning("No LSB payload found.")
        except Exception as e:
            logger.error(f"LSB extraction error: {e}")

        # 2. Try Atom Layer (File Info)
        try:
...
class MP5Verifier:
    ...
    def verify(self, mp5_path: str) -> Dict[str, Any]:
        result = {"file": mp5_path, "overall": "unknown"}
        try:
            data = self.decoder.decode(mp5_path)
            
            if data.get("hidden_payload"):
                result["overall"] = "verified"
                result["payload_status"] = "present"
            else:
                result["overall"] = "failed"
                result["payload_status"] = "missing"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("\nMP5 MVP Utility")
        print("Usage:")
        print("  Encode: python mp5.py encode <input.mp4> <metadata.json> <output.mp4>")
        print("  Decode: python mp5.py decode <input.mp5.mp4>")
        print("  Verify: python mp5.py verify <input.mp5.mp4>")
        sys.exit(1)
    ...
    try:
        if command == "encode":
            if len(sys.argv) != 5:
                print("Error: Usage is `encode <input> <json_file> <output>`")
                sys.exit(1)
            
            input_video, meta_file, output_video = sys.argv[2], sys.argv[3], sys.argv[4]
            
            with open(meta_file, 'r') as f:
                metadata = json.load(f)
                
            encoder = MP5Encoder(config)
            res = encoder.encode(input_video, metadata, output_video)
            ...
            # Print specifically the hidden payload for convenience
            if "hidden_payload" in res:
                print(json.dumps(res["hidden_payload"], indent=2))
            else:
                print(json.dumps(res, indent=2))
